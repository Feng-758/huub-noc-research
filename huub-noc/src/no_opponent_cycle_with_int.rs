	...
	E: ReasoningEngine,
{
	...
	fn propagate(
		...
		let mut v_val: Vec<Option<i64>> = vec![None; n];
		for v in 0..n {
...
impl<I> NoOpponentCycleWithInt<I> {
	...
	#[inline]
	fn is_active(&self, v: usize, v_val: Option<i64>) -> bool {
		let Some(x) = v_val else { return false };
		let x: i64 = x.into();

		if self.is_even(v) { x != 0 } else { x == 1 }
	}

	#[inline]
	fn value_for_edge(&self, e: usize) -> i64 {
		(e as i64 + 1).into()
	}

	#[inline]
	fn chosen_edge_of_even(&self, v: usize, vv: Option<i64>) -> Option<usize> {
		let val = vv?;
		...
		I: IntSolverActions<E>,
	{
		...
		if in_stack[v] {
			...
			if self.cycle_is_forbidden_by_parity_vertices(cycle_vertices) {
				...
				// Do NOT include parent_edge itself, otherwise the propagation is
				// self-justifying.
		...
		if self.is_even(v) {
			...
			if let Some(e) = self.chosen_edge_of_even(v, Some(vv)) {
				...
				self.noceager_dfs_int::<E>(ctx, v_val, path_v, path_e, in_stack, w, Some(e))?;
			}
		} else {
